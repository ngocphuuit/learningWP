...
class OxygennaUpload extends OxygennaOption
{
    ...
    private function create_option($value, $url)
    {
        ...
        $option .= '<input type="button" class="oxy-set-image" data-frame-title="' . __('Select Image', 'angle-admin-td') . '" data-frame-button-text="' . __('Select Image', 'angle-admin-td') . '"  value="' . __('Set Image', 'angle-admin-td') . '"/>';
        $option .= '<input type="button" class="oxy-remove-image" value="' . __('Remove Image', 'angle-admin-td') . '" style="' . $hide_preview . '"/>';
        $option .= '</div>';
