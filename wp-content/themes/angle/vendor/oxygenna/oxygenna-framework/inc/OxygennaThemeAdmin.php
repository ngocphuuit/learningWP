...
class OxygennaThemeAdmin
{
    ...
    { ?>
        <div class="error">
            <p><?php _e('Looks like you have the <strong>Oxygenna Theme Framework Plugin</strong> Installed - this is no longer needed please delete it <a href="' . admin_url('plugins.php') . '">here on your plugins page</a>', 'angle-admin-td'); ?></p>
        </div>
    <?php
    }
    ...
    public function check_theme_compatible()
    {
        ...
        if (version_compare($version, $this->options['min_wp_ver'], '<')) {
            $this->errors[] = sprintf(__('Version %s is incompatible with this theme minimum version %s', 'angle-admin-td'), $version, $this->options['min_wp_ver']);
        }
